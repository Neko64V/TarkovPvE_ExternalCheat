...
{
    // Get ClientSize
    GetClientRect(g.GameHwnd, &g.GameSize);
    ClientToScreen(g.GameHwnd, &g.GamePoint);

    // Create Overlay
    wc = { sizeof(WNDCLASSEXA), 0, WndProc, 0, 0, NULL, NULL, NULL, NULL, Title, Class, NULL};
    RegisterClassExA(&wc);
    Hwnd = CreateWindowExA(WS_EX_LAYERED | WS_EX_TRANSPARENT | WS_EX_TOOLWINDOW, wc.lpszClassName, wc.lpszMenuName, WS_POPUP | WS_VISIBLE, g.GamePoint.x, g.GamePoint.y, g.GameSize.right, g.GameSize.bottom, NULL, NULL, wc.hInstance, NULL);
...
void Overlay::OverlayManager()
{
    ...
    {
        // Window Check
        HWND CheckHwnd = FindWindowA(nullptr, "EscapeFromTarkov");
        if (!CheckHwnd)
        {
            g.Run = false;
            break;
